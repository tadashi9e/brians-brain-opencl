...
// cl kernel
// ----------------------------------------------------------------------
static const char *kernel_source = "brians_brain_kernel.cl";
...
static cl::Kernel bb_kernel_next_step;
static cl::Kernel bb_kernel_draw_image;
...
static char title[] = "Brian's Brain on OpenCL";
...
static void generationTimer_cb(int dummy) {
  ...
  try {
    ...
    command_queue.enqueueNDRangeKernel(bb_kernel_next_step,
                                       cl::NullRange,
    ...
    command_queue.enqueueNDRangeKernel(bb_kernel_draw_image,
                                       cl::NullRange,
                                       cl::NDRange(global_work_size[0],
                                                   global_work_size[1]),
                                       cl::NDRange(local_work_size[0],
...
void bb_init_random_field(std::vector<cl_char>& field_init) {
...
#define ST_ON 2
        const cl_char c = rand_r(&seed) % (ST_ON + 1);
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    bb_init_random_field(field_init);
    /* end init field_init */
